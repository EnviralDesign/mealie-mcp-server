...
- Local OpenAPI baseline: `247` operations (`openapi.json`)
- Live Unraid Mealie checked 2026-05-18: Mealie `v3.17.0`, `249` operations
  - The live API adds streaming recipe create routes not present in the local snapshot.
  - Do not assume the checked-in OpenAPI snapshot is current.
- MCP profile counts:
  - `core`: `65` tools
  - `full`: `75` tools
...
| Tools (equipment) | 6 | 7 | `get_tools`, `get_tool`, `create_tool`, `update_tool`, `delete_tool`, `set_recipe_tools` (+ `get_tool_by_slug` in full) |
| Recipe cleanup helpers | 8 | 8 | `set_recipe_organizers`, `set_recipe_instructions`, `get_recipe_ingredient_rows`, `update_recipe_ingredient_row`, `update_recipe_ingredient_rows`, `add_recipe_ingredient_row`, `delete_recipe_ingredient_row`, `get_recipe_cleanup_summary` |
| Foods | 7 | 7 | `get_foods`, `get_food`, `create_food`, `update_food`, `get_or_create_food`, `delete_food`, `merge_foods` |
| Units | 6 | 6 | `get_units`, `get_unit`, `create_unit`, `update_unit`, `delete_unit`, `merge_units` (create/update expose plural display metadata) |
...
These routes already have client methods in `src/mealie_mcp/client.py` and are low effort to expose.
...
- Streaming recipe create endpoints: `/api/recipes/create/url/stream`, `/api/recipes/create/html-or-json/stream`
